...
fn main() {
    App::new()
        ...
        .add_systems(Startup, (setup_camera, mesh))
        .run();
}

fn setup_camera(mut comds: Commands) {
    // Spawn the camera
    comds.spawn(Camera2dBundle::default());
}
...
fn mesh(mut commands: Commands, mut meshes: ResMut<Assets<Mesh>>, mut windows: Query<&mut Window>) {
    ...
    let v_color: Vec<u32> = vec![
        LinearRgba::BLACK.as_u32(),
        LinearRgba::from(bevy::color::palettes::basic::RED).as_u32(),
        LinearRgba::from(bevy::color::palettes::basic::GREEN).as_u32(),
        LinearRgba::from(bevy::color::palettes::basic::BLUE).as_u32(),
    ];
...
/// Extract the [`ColoredMesh2d`] marker component from main world into the render world
